...
proto
    : syntax (importStatement | packageStatement | optionStatement | topLevelDef | emptyStatement)* EOF
    ;
...
field
    : fieldLabel? type fieldName EQ fieldNumber (LB fieldOptions RB)? SEMI
    ;
...
mapField
    : MAP LT keyType COMMA type GT mapName EQ fieldNumber (LB fieldOptions RB)? SEMI
    ;
...
extendDef
    : EXTEND messageType LC (field | emptyStatement)* RC
    ;
...
rpc
    : RPC rpcName LP (STREAM)? messageType RP RETURNS LP (STREAM)? messageType RP (
        LC ( optionStatement | emptyStatement)* RC
        | SEMI
    )
    ;
